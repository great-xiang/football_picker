...
class YOLO(object):
    ...
    #   检测图片
    #---------------------------------------------------#
    def detect_image(self, image, crop = False, count = False,save_img=False):
        #---------------------------------------------------#
        #   获得输入图片的高和宽
        #---------------------------------------------------#
        ...
        with torch.no_grad():
            ...
            for i in range(len(top_label)):
                my_result.append(top_label[i])
                my_result.append(top_conf[i])
                my_result.append(224-top_boxes[i][2])
                my_result.append((top_boxes[i][1]+top_boxes[i][3])/2-112)
