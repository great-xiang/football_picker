...
for i in range(1000000):
    ...
    agent.learn(obs, act_log_prob, reward)
    avr+=reward
    if (i % 1000 == 0):
        frequency = int(i / 1000)
        avr/=1000
        print("过了%d次" % frequency)
        print("angle:", angle, "speed:", speed, "Angle:", int(Angle), "角度差：", int(Angle - float(action[0] * 90)),"reward:", int(avr))
