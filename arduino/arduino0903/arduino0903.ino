...
int angle, serialData[4], i, temp = 0, timer_flag = 0;
...
// PID参数设置
double Input0 = 0, Input1 = 0, Output0 = 0, Output1 = 0, Setpoint0 = 0, Setpoint1 = 0;
double errSum0 = 0, errSum1 = 0, lastErr0 = 0, lastErr1 = 0;
// P主管响应，I减小静差，D抑制震荡。
// p比例，p很小，柔软；p很大，生硬。
...
void read_uart()
{
    ...
    if (Serial.available() >= 16)
    {
        ...
        //读取串口缓存中等待的字符,并赋值给字符变量serialData
        for (i = 0; i < 4; i++)
        {
            serialData[i] = (Serial.read() - 48) * 100;
            serialData[i] += (Serial.read() - 48) * 10;
        ...
        if (serialData[0] != 0)
        {
            Setpoint0 = -serialData[0];
        }
        else
        {
            Setpoint0 = serialData[1];
        }
        if (serialData[2] != 0)
        {
            Setpoint1 = -serialData[2];
        }
        else
        {
            Setpoint1 = serialData[3];
        }
        if (Setpoint0 == 0)
        {
            analogWrite(zuoqian, 0);
            analogWrite(zuohou, 0);
            Input0 = 0, Output0 = 0, errSum0 = 0, lastErr0 = 0;
        }
        if (Setpoint1 == 0)
        {
            analogWrite(youqian, 0);
            analogWrite(youhou, 0);
            Input1 = 0, Output1 = 0, errSum1 = 0, lastErr1 = 0;
        }
        if (trig0)
        {
            trig0 = getball();
...
void stop()
{
    ...
    analogWrite(youqian, 0);
    analogWrite(youhou, 0);
    Setpoint0 == 0, Setpoint1 == 0, Input1 = 0, Output1 = 0, errSum1 = 0, lastErr1 = 0, Input0 = 0, Output0 = 0, errSum0 = 0, lastErr0 = 0;
}
//使用PID产生的output控制电机
void control()
{
...
void Compute0()
{
    double error0 = Setpoint0 - Input0;
    errSum0 += error0;
    double dErr0 = error0 - lastErr0;
    Output0 = int(kp * error0 + ki * errSum0 + kd * dErr0);
    lastErr0 = error0;
}
...
void Compute1()
{
    ...
    Output1 = int(kp * error1 + ki * errSum1 + kd * dErr1);
    lastErr1 = error1;
}
...
void send_uart()
{
    ...
    Serial.print(",");
    Serial.print(int(Setpoint0), DEC);
    Serial.print(",");
    Serial.print(int(Setpoint1), DEC);
    Serial.print(",");
    Serial.print(int(rpm0), DEC);
    Serial.print(",");
    Serial.print(int(rpm1), DEC);
    Serial.print(",");
    Serial.print(int(Output0), DEC);
    Serial.print(",");
    Serial.print(int(Output1), DEC);
    Serial.print(",");
    Serial.print("b");
    Serial.print(",");
}
...
void read_speed()
{
    rpm0 = int(number0 / 0.39); //减速比21.3，编码器精度11，num/11/21.3*120 num/11/21.3*10*60
    number0 = 0;
    rpm1 = int(number1 / 0.39);
    number1 = 0;
}
