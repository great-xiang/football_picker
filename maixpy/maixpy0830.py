...
def arduino(self):
    ...
    if (read_data != None):
        ...
        try:
            ...
            print('磁场度数:%3s' % data[0], '真实速度:%4s' % data[1], '%4s' % data[2], 'pwm%3s' % data[3], '%3s' % data[4],
                  '%3s' % data[5], '%3s' % data[6])
        except:
            pass
...
# ————————————————————————————
def detect_line():
    img = sensor.snapshot().mean(1).binary([THRESHOLD],invert=True)
    line = img.get_regression([(100,100)], roi=ROI,robust = True)
    if (line):
        rho_err = (abs(line.rho()) - img.width() / 2)
        turn_info = rho_err / 64
