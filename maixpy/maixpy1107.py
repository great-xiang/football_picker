...
fm.register(4, fm.fpioa.UART1_TX, force=True)
fm.register(5, fm.fpioa.UART1_RX, force=True)
uart3 = UART(UART.UART2, 115200, 8, None, 0, timeout=1000, read_buf_len=4096)
...
#class PID():
    #def __init__(self, name):
        ## PID参数设置
        #self.kp, self.ki = 3, 0.6
        #self.lastpwm=0
        #self.name = name

    #def run(self, Set_rpm, real_rpm):
        #error = Set_rpm - real_rpm
        #pwm = int(self.kp * error + self.ki*self.lastpwm)
        #self.lastpwm = pwm
        #if (pwm > 255):
            #pwm = 255
        #if (pwm < -255):
...
fm.register(0, fm.fpioa.UART1_TX, force=True)
fm.register(1, fm.fpioa.UART1_RX, force=True)
uart3 = UART(UART.UART1, 115200, 8, None, 0, timeout=1000, read_buf_len=4096)
