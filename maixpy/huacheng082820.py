...
# ——————————————————————————————————————————————————————————————————————————————————————————————
def send_speed(v0=100, det_v=150, turn_info=0):
    global speed
    if (turn_info < 0):
...
def maganet():
    ...
    if (read_data != None):
        data = str(read_data)
        mag = int(data[2:data.index(',')])
        print('磁场度数:', mag, 'Setpoint、Outpoint、rpm:', data[data.index(',') + 1:-1])
